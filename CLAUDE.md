...
* Every type compiles under every supported configuration, `Release.R20` through `Release.R27`. `System.Threading.Lock`, `ReplaceLineEndings` and async disposal of a `FileStream` are unavailable on the .NET Framework configurations without Polyfill.
* `Nice3point.TUnit.Revit` starts at Revit 2021. The solution maps the test project onto `R21` under the `R20` configurations and drops it from that build.
